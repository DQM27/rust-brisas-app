...
/// Calcula los minutos transcurridos desde una fecha de ingreso hasta AHORA (UTC).
pub fn calcular_tiempo_desde_ingreso(fecha_ingreso_str: &str) -> Result<i64, CommonError> {
    let ingreso = DateTime::parse_from_rfc3339(fecha_ingreso_str)
        .map_err(|_| CommonError::FechaIngresoInvalida)?;

    let ahora = chrono::Utc::now();
    let duracion = ahora.signed_duration_since(ingreso);
    Ok(duracion.num_minutes())
}
