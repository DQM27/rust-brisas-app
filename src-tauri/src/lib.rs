...
pub fn run() {
    #[cfg(not(mobile))]
    {
        ...
        main_inner().expect("Error al iniciar la aplicación");
    }
}

/// Genera todos los handlers de comandos de forma organizada
fn generate_handlers() -> impl Fn(tauri::Invoke<tauri::Wry>) + Send + Sync + 'static {
