...
use crate::models::vehiculo::{
    VehiculoResponse, VehiculoListResponse, TipoVehiculoStats, TipoVehiculo,
    CreateVehiculoInput, UpdateVehiculoInput,
};
use sqlx::SqlitePool;
use uuid::Uuid;
use chrono::Utc;
...
pub async fn create_vehiculo(
    ...
    let marca_normalizada = input.marca
        .as_ref()
        .map(|m| domain::normalizar_texto(m))
        .filter(|m| !m.is_empty());
    
    let modelo_normalizado = input.modelo
        .as_ref()
        .map(|m| domain::normalizar_texto(m))
        .filter(|m| !m.is_empty());
    
    let color_normalizado = input.color
        .as_ref()
...
pub async fn get_all_vehiculos(pool: &SqlitePool) -> Result<VehiculoListResponse, String> {
    ...
    let motocicletas = vehiculo_responses.iter()
        .filter(|v| v.tipo_vehiculo == TipoVehiculo::Motocicleta)
        .count();
    let automóviles = vehiculo_responses.iter()
