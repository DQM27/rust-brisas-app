...
use crate::domain::ingreso::strategy::{get_strategy, IngresoStrategy};
...
use crate::models::ingreso::{
    ...
    IngresoResponse, ModoIngreso, TipoAutorizacion, TipoIngreso, ValidacionIngresoResponse,
};
...
pub async fn crear_ingreso_visita(
    ...
    // Crear DatosValidacion a partir del input
    let datos = DatosValidacion {
        cedula: input.cedula.clone(),
        nombre: Some(input.nombre.clone()),
        apellido: Some(input.apellido.clone()),
        tipo_ingreso: TipoIngreso::Visita.as_str().to_string(),
        contratista_id: None,
        anfitrion: Some(input.anfitrion.clone()),
        area_visitada: Some(input.area_visitada.clone()),
        motivo_visita: Some(input.motivo_visita.clone()),
        empresa_id: None,
        motivo_proveedor: None,
        tipo_autorizacion: input.tipo_autorizacion.clone(),
        modo_ingreso: input.modo_ingreso.clone(),
        gafete_numero: input.gafete_numero.clone(),
        vehiculo_id: None,
        placa_temporal: input.vehiculo_placa.clone(),
    };

    // Obtener estrategia
    let strategy = get_strategy(&TipoIngreso::Visita);

    // Validar entrada
    strategy.validar_entrada(pool, &datos).await?;

    // Preparar datos para inserción
    let datos_ingreso = strategy
        .preparar_datos_ingreso(pool, &datos, &usuario_id)
        .await?;
    ...
    db::insert_extended(
        ...
        &datos_ingreso.id,
        None,
        &datos_ingreso.cedula,
        &datos_ingreso.nombre,
        &datos_ingreso.apellido,
        &datos_ingreso.empresa_nombre,
        None,
        datos_ingreso.anfitrion.as_deref(),
        datos_ingreso.area_visitada.as_deref(),
        datos_ingreso.motivo_visita.as_deref(),
        None,
        TipoIngreso::Visita.as_str(),
        tipo_autorizacion.as_str(),
        modo_ingreso.as_str(),
        None,
        datos_ingreso.placa_temporal.as_deref(),
        datos_ingreso.gafete_numero.as_deref(),
        &now,
        &usuario_id,
        None,
        None,
    ...
    // Retornar ingreso creado
    get_ingreso_by_id(pool, datos_ingreso.id).await
}
...
pub async fn crear_ingreso_proveedor(
    ...
    let datos = DatosValidacion {
        ...
        area_visitada: Some(input.area_visitada.clone()),
        motivo_visita: None,
        empresa_id: Some(input.empresa_id.clone()),
        motivo_proveedor: Some(input.motivo.clone()),
        tipo_autorizacion: input.tipo_autorizacion.clone(),
        modo_ingreso: input.modo_ingreso.clone(),
        gafete_numero: input.gafete_numero.clone(),
        vehiculo_id: None,
    ...
    // Obtener estrategia
    let strategy = get_strategy(&TipoIngreso::Proveedor);
    ...
    db::insert_extended(
        ...
        &datos_ingreso.apellido,
        &datos_ingreso.empresa_nombre,
        datos_ingreso.empresa_proveedor_id.as_deref(),
        None,
        datos_ingreso.area_visitada.as_deref(),
        None,
        datos_ingreso.motivo_proveedor.as_deref(),
        TipoIngreso::Proveedor.as_str(),
        tipo_autorizacion.as_str(),
        modo_ingreso.as_str(),
        None,
        datos_ingreso.placa_temporal.as_deref(),
