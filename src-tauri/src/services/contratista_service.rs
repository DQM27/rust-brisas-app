    ...
    A: AuditRepository,
{
    ...
    pub async fn create_contratista(
        ...
        // Note: domain::validar_fecha now returns DateTime<Utc> directly per domain refactor
        let fecha_vencimiento: DateTime<Utc> =
            domain::validar_fecha(&input.fecha_vencimiento_praind)?;
    ...
    pub async fn update_contratista(
        ...
        if let Some(v) = input.fecha_vencimiento_praind {
            // Note: domain::validar_fecha now returns DateTime<Utc>
            let fecha: DateTime<Utc> = domain::validar_fecha(&v)?;
        ...
        if input.tiene_vehiculo == Some(true) {
            if let (Some(tipo), Some(placa)) = (&input.tipo_vehiculo, &input.placa) {
                if !tipo.is_empty() && !placa.is_empty() {
                    ...
                    // NOTE: This direct DB access might break the repository pattern abstraction
                    // but keeping it as per HEAD implementation until full refactor.
                    // Ideally should use self.veh_repo methods if available.
    ...
    pub async fn actualizar_praind_con_historial(
        ...
        let nueva_fecha: DateTime<Utc> = domain::validar_fecha(&input.nueva_fecha_praind)?;
