...
#[command]
pub fn set_window_decorations(window: Window, decorations: bool) -> Result<(), SystemError> {
    window.set_decorations(decorations).map_err(|e| SystemError::Window(e.to_string()))
}

/// Ajusta las dimensiones físicas de la ventana de la aplicación.
#[command]
pub fn set_window_size(window: Window, width: f64, height: f64) -> Result<(), SystemError> {
    window
        .set_size(tauri::Size::Logical(tauri::LogicalSize { width, height }))
        .map_err(|e| SystemError::Window(e.to_string()))
}
