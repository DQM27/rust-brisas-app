...
pub async fn find_historial_fetched(
    ...
    let mut result = db
        .query(format!(
            "SELECT * FROM {TABLE} WHERE fecha_hora_ingreso >= type::datetime($start) AND fecha_hora_ingreso <= type::datetime($end) ORDER BY fecha_hora_ingreso DESC FETCH usuario_ingreso, usuario_salida, proveedor, proveedor.empresa"
        ))
        .bind(("start", start))
        .bind(("end", end))
        .await?;
