...
pub async fn create_empresa(input: CreateEmpresaInput) -> Result<Empresa, SurrealDbError> {
...
pub async fn get_empresa_by_id(id: &str) -> Result<Option<Empresa>, SurrealDbError> {
    let db = get_surrealdb().ok_or(SurrealDbError::NotConnected)?;
    let client = db.get_client().await?;

    // Normalizar ID (quitar prefijo si existe)
...
pub async fn get_all_empresas() -> Result<Vec<Empresa>, SurrealDbError> {
    let db = get_surrealdb().ok_or(SurrealDbError::NotConnected)?;
    let client = db.get_client().await?;
...
pub async fn delete_empresa(id: &str) -> Result<(), SurrealDbError> {
    let db = get_surrealdb().ok_or(SurrealDbError::NotConnected)?;
    let client = db.get_client().await?;
...
pub async fn count_contratistas_by_empresa(_empresa_id: &str) -> Result<usize, SurrealDbError> {
    // Placeholder - se implementará cuando el módulo contratistas esté listo
    Ok(0)
}
