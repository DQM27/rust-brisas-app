...
pub async fn find_by_id(pool: &SqlitePool, id: &str) -> Result<Ingreso, String> {
    let row = sqlx::query(
        "SELECT id, contratista_id, cedula, nombre, apellido, empresa_nombre,
         ...
         FROM ingresos 
         WHERE id = ? AND tipo_ingreso = 'contratista'", // Filtramos por tipo para seguridad
    )
    .bind(id)
    .fetch_one(pool)
    .await
    .map_err(|_| "Ingreso de contratista no encontrado".to_string())?;
...
pub async fn find_all(pool: &SqlitePool) -> Result<Vec<Ingreso>, String> {
    let rows = sqlx::query(
        "SELECT id, contratista_id, cedula, nombre, apellido, empresa_nombre,
         ...
         ORDER BY fecha_hora_ingreso DESC LIMIT 500",
    )
    .fetch_all(pool)
    .await
...
pub async fn insert(
    pool: &SqlitePool,
    id: &str,
    contratista_id: &str, // Requerido para contratista
    cedula: &str,
    nombre: &str,
    apellido: &str,
    empresa_nombre: &str,
    ...
    .bind(id)
    .bind(Some(contratista_id)) // bind explicito
    .bind(cedula)
    .bind(nombre)
    .bind(apellido)
    .bind(empresa_nombre)
    ...
    .execute(pool)
    .await
    .map_err(|e| format!("Error al crear ingreso de contratista: {}", e))?;
