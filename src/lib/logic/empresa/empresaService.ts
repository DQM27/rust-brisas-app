...
import { crearEmpresa, actualizarEmpresa, eliminarEmpresa, fetchTodasEmpresas, fetchEmpresasActivas } from '$lib/api/empresa';
import type { EmpresaResponse, EmpresaListResponse, CreateEmpresaInput, UpdateEmpresaInput } from '$lib/types/empresa';
...
function validateEmpresaInput(nombre: string): { ok: true } | { ok: false; message: string } {
  ...
  if (n.length > 100) return { ok: false, message: 'El nombre de la empresa no puede exceder 100 caracteres.' };
  return { ok: true };
}
...
async function updateEmpresaService(id: string, input: UpdateEmpresaInput): Promise<EmpresaResponse> {
...
export type SubmitCreateEmpresaResult = { ok: true; empresa: EmpresaResponse } | { ok: false; error: string };
export type SubmitUpdateEmpresaResult = { ok: true; empresa: EmpresaResponse } | { ok: false; error: string };
export type SubmitDeleteEmpresaResult = { ok: true } | { ok: false; error: string };
export type SubmitFetchAllEmpresasResult = { ok: true; empresas: EmpresaListResponse } | { ok: false; error: string };
export type SubmitFetchActiveEmpresasResult = { ok: true; empresas: EmpresaResponse[] } | { ok: false; error: string };
...
export async function submitUpdateEmpresa(id: string, nombre: string): Promise<SubmitUpdateEmpresaResult> {
  const validation = validateEmpresaInput(nombre);
  if (!validation.ok) return { ok: false, error: validation.message };
