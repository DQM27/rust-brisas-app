...
export const ingreso = {
	...
	registrarSalida: async (input: RegistrarSalidaInput): Promise<IngresoResponse> => {
		...
		return await invoke('register_exit_contratista', {
			input: validated,
			usuarioId: input.usuarioSalidaId || 'unknown'
		});
	},
	 ...
	 */
	registrarSalidaConGafete: async (
		_ingresoId: string,
		_gafeteNumero: string,
		_usuarioSalidaId: string
	): Promise<IngresoResponse> => {
		...
		throw new Error(
			'Este endpoint requiere migración a register_exit_contratista con parámetros adecuados'
		);
	},
