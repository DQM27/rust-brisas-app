<script lang="ts">
	...
	const headerColor = $derived(
		type === 'danger' ? 'text-red-400' : type === 'warning' ? 'text-orange-400' : 'text-blue-400'
	);

	const iconColor = $derived(
		type === 'danger' ? 'text-red-500' : type === 'warning' ? 'text-orange-500' : 'text-blue-500'
	);
	...
	>
		...
		>
			...
			<div class="flex items-center justify-between border-b border-white/5 bg-white/5 px-6 py-4">
				<h3 id="modal-title" class="text-lg font-semibold {headerColor} flex items-center gap-2">
			...
			<!-- Footer -->
			<div class="flex justify-end gap-3 border-t border-white/5 bg-[#252526] px-6 py-4">
				<button
					onclick={handleClose}
					type="button"
