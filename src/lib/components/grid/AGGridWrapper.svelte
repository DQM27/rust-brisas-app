<script lang="ts" generics="T extends Record<string, any>">
	...
	import { themeQuartz, colorSchemeDark, colorSchemeLight } from '@ag-grid-community/theming';
	import type { GridOptions, GridApi } from '@ag-grid-community/core';
	import type { ToolbarContext, AGGridWrapperProps } from '$lib/types/agGrid';
	...
	const suppressRowClickSelection = $derived(agGridSettings.getSuppressRowClickSelection(gridId));
	...
	const suppressContextMenu = $derived(agGridSettings.getSuppressContextMenu(gridId));
	const autoSizeOnLoad = $derived(agGridSettings.getAutoSizeColumnsOnLoad(gridId));

	// Tema personalizado reactivo
	const myTheme = $derived.by(() => {
		const isDark = currentTheme.includes('dark');
		const baseTheme = themeQuartz.withPart(isDark ? colorSchemeDark : colorSchemeLight);
		...
		return baseTheme.withParams({
			...
			rowHoverColor: isDark ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.05)',
			selectedRowBackgroundColor: isDark ? 'rgba(59, 130, 246, 0.15)' : 'rgba(59, 130, 246, 0.1)',
			columnBorder: true,
	...
	$effect(() => {
		if (gridApi) {
			gridApi.setGridOption('enterNavigatesVertically', enterNavigation === 'down');
			gridApi.setGridOption('enterNavigatesVerticallyAfterEdit', enterNavigation === 'down');
		}
	});
	...
	function debouncedSaveColumnState(api: GridApi) {
		...
		columnEventTimeout = setTimeout(() => gridState.saveColumnState(api, persistenceKey), 300);
	}
    ...
    `,
		...
		onGridReady: (params) => {
			...
			setTimeout(async () => {
				...
				let restored = false;
				if (persistenceKey) {
					restored = await gridState.restoreColumnState(params.api, persistenceKey);
				}
		...
		onColumnResized: (params) => {
			if (!persistenceKey) return;
			if (params.source === 'autosizeColumns' || params.source === 'sizeColumnsToFit') {
