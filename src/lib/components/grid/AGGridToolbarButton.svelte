<script lang="ts">
	...
	let { button, disabled = false, onclick }: Props = $props();
	...
	const variantClasses = $derived.by(() => {
		const base =
			'flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium cursor-pointer transition-all duration-200 whitespace-nowrap';
