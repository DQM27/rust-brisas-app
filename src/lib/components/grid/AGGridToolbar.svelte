...
<script lang="ts">
	...
	const visibleButtons = $derived.by(() => {
		...
		if (order.length === 0) {
			...
			commonBtns = allButtons.filter((btn) => !btn.custom && !hidden.includes(btn.id));
		} else {
			// Ordenar según configuración y filtrar ocultos
			commonBtns = order
				.map((id) => allButtons.find((btn) => btn.id === id && !btn.custom))
				.filter((btn): btn is UnifiedButton => btn !== undefined && !hidden.includes(btn.id));
		}
		...
		switch (buttonId) {
			...
			case 'expand-groups':
				gridApi.forEachNode((node) => {
					if (node.group || (node.childrenAfterGroup && node.childrenAfterGroup.length > 0)) {
	...
	const categoryOrder = ['custom', 'columns', 'export', 'selection', 'data', 'ui'];
	...
	<div class="flex items-center gap-2">
		{#each orderedGroups as group, idx}
			...
			{#each group.buttons as button}
				<div data-filter-button={button.id.startsWith('filter-') ? 'true' : undefined}>
					<AGGridToolbarButton
						{button}
