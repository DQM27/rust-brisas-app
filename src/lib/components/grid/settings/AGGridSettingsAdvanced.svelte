<script lang="ts">
	...
	let suppressContextMenu = $derived(agGridSettings.getSuppressContextMenu(gridId));
	let enableUndoRedo = $derived(agGridSettings.getEnableUndoRedo?.(gridId) ?? false);
	let rowBuffer = $derived(agGridSettings.getRowBuffer?.(gridId) ?? 10);
	let debounceScroll = $derived(agGridSettings.getDebounceScroll?.(gridId) ?? true);
	...
	const groupClass = 'space-y-3 p-3 rounded-lg bg-black/10 border border-white/5';
</script>
		...
		<div class="space-y-4">
			...
			<div class={groupClass}>
				...
				<label class="flex items-center gap-3 cursor-pointer group">
					...
					<div class="flex flex-col">
						<span class="text-xs text-zinc-200 group-hover:text-white flex items-center gap-2">
							<Undo2 size={12} class="text-purple-400" /> Deshacer / Rehacer
						</span>
						<span class="text-[10px] text-zinc-500">Habilitar Ctrl+Z / Ctrl+Y</span>
					</div>
				</label>
				...
				<label class="flex items-center gap-3 cursor-pointer group">
					...
					/>
					<div class="flex flex-col">
						<span class="text-xs text-zinc-200 group-hover:text-white">Copiar Encabezados</span>
						<span class="text-[10px] text-zinc-500">Incluir al copiar filas</span>
					</div>
				</label>
				...
				<label class="flex items-center gap-3 cursor-pointer group">
					...
					/>
					<div class="flex flex-col">
						<span class="text-xs text-zinc-200 group-hover:text-white">Ocultar Menú AG Grid</span>
						<span class="text-[10px] text-zinc-500">Usar menú nativo del navegador</span>
					</div>
				</label>
			</div>
			...
			<div class={groupClass}>
				...
				<label class="flex items-center gap-3 cursor-pointer group">
					...
					/>
					<span
						class="text-xs text-zinc-200 group-hover:text-white {dontAskAgain ? 'opacity-50' : ''}"
				...
				<label class="flex items-center gap-3 cursor-pointer group pt-1 border-t border-white/5">
					<input
						type="checkbox"
			...
			<div>
				...
				<div class={groupClass}>
					<div class="space-y-2">
						...
						<div class="grid grid-cols-4 gap-1" role="group" aria-label="Buffer de filas">
					...
					<label class="flex items-center gap-3 cursor-pointer group mt-2">
						...
						/>
						<div class="flex flex-col">
							<span class="text-xs text-zinc-200 group-hover:text-white">Suavizar Scroll</span>
							<span class="text-[10px] text-zinc-500">Reduce parpadeo al scrollear</span>
						</div>
					</label>
				</div>
			</div>
