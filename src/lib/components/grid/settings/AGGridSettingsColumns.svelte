...
<div class={sectionClass}>
	...
	<div class="space-y-2">
		<div class="flex items-center justify-between px-1">
			...
			{#if pinnedLeftCount > 0 || pinnedRightCount > 0}
				<span class="text-[10px] text-zinc-500">
					<span class="text-blue-400 font-medium">{pinnedLeftCount + pinnedRightCount}</span> fijadas
				</span>
			{/if}
		</div>
	...
	<div class="border border-white/5 rounded-lg bg-black/10 overflow-hidden">
		<div class="max-h-[300px] overflow-y-auto custom-scrollbar p-1 space-y-0.5" role="list">
          ...
          {draggedIndex === index
              ...
              {column.visible
					...
					<!-- Name -->
					<span
						class="flex-1 text-xs truncate {column.visible ? 'text-zinc-300' : 'text-zinc-500'}"
					>
						{column.name}
					</span>
