<script lang="ts">
  ...
  const contextDescriptions: Record<ToolbarContext, string> = {
    ...
    singleSelect:
      "Botones visibles cuando hay exactamente una fila seleccionada",
...
<div class={sectionClass}>
  ...
  >
    <div class="flex items-center gap-3">
      <div
        class="p-1 rounded-md {isOverLimit
          ? 'bg-red-500/20'
          : 'bg-green-500/20'}"
      >
        <Tally5
  ...
  <!-- Draggable List -->
  <div>
    <h3 class="text-xs font-medium text-zinc-400 mb-2 ml-0.5">
      Orden y Visibilidad
    </h3>
    <div
      class="space-y-1 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar"
      role="list"
    >
