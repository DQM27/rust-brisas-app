<script lang="ts">
	...
	let theme = $state<AGGridTheme>(untrack(() => agGridSettings.getTheme(gridId)));
	let font = $state<AGGridFont>(untrack(() => agGridSettings.getFont(gridId)));
	let rowHeight = $state<RowHeight>(untrack(() => agGridSettings.getRowHeight(gridId)));
	let headerHeight = $state<number>(untrack(() => agGridSettings.getHeaderHeight(gridId)));
	let toolbarPosition = $state<ToolbarPosition>(
		untrack(() => agGridSettings.getToolbarPosition(gridId))
	);
	let animateRows = $state<boolean>(untrack(() => agGridSettings.getAnimateRows(gridId)));
...
<div class={sectionClass}>
	...
	<div class="grid grid-cols-2 gap-4">
		...
		<!-- Font -->
		<SelectDropdown label="Tipografía" value={font} options={fonts} onSelect={handleFontChange} />
	</div>
	...
	<div class="pt-2">
		<div class="flex justify-between items-center mb-2">
			<label class={labelClass} for="{appearanceId}-header-height">Altura de Cabecera</label>
			<span
		...
		<div class="px-1">
			...
			/>
			<div class="flex justify-between text-[10px] text-zinc-600 mt-1.5 font-medium px-0.5">
	...
	<div class="grid grid-cols-2 gap-4">
		<label class="flex items-center gap-3 cursor-pointer group">
			...
			/>
			<span class="text-sm text-zinc-400 group-hover:text-zinc-200">Animar filas</span>
		</label>
		...
		<label class="flex items-center gap-3 cursor-pointer group">
			<input
				...
				class="w-4 h-4 rounded bg-black/20 border-zinc-600 text-blue-600 focus:ring-blue-600 focus:ring-offset-0 transition-all checked:bg-blue-600 checked:border-blue-600"
			/>
			<span class="text-sm text-zinc-400 group-hover:text-zinc-200">Selección de texto</span>
		</label>
	</div>
</div>
