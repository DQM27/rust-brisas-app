<script lang="ts">
	...
	let { userId, currentPassword = '', onSuccess, onCancel, hideHeader = false }: Props = $props();
...
<div class="w-full flex flex-col p-4 px-6">
	{#if !hideHeader}
		<div class="mb-4 text-center">
			<h2 class="text-lg font-bold text-gray-900 dark:text-gray-100">Cambiar Contraseña</h2>
	...
	<form onsubmit={handleSubmit} class="space-y-4">
		...
		<div class={currentPassword ? 'hidden' : 'block'}>
			<label for="current_password" class="block text-sm font-medium text-secondary mb-1"
		...
		<!-- Nueva Contraseña -->
		<div>
			<label for="new_password" class="block text-sm font-medium text-secondary mb-1"
