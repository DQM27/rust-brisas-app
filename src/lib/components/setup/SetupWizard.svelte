<script lang="ts">
	...
	async function handleRecoverFromFragments() {
		...
		try {
			...
			if (!paths || !Array.isArray(paths) || paths.length < 3) {
				if (paths) {
					await message('Se requieren al menos 3 fragmentos para la recuperación.', {
			...
			await message('Sistema recuperado exitosamente usando fragmentos de Shamir.', {
	...
	async function handleFactoryReset() {
		...
		isConfirming = true;
		try {
			const confirmed = await ask('¿Estás seguro? Esto borrará TODA la configuración.', {
	...
	{#if isResetting}
		<div class="flex flex-col items-center gap-4 text-center" transition:fade>
			...
			<div>
				...
				<p class="text-sm text-secondary">Limpiando configuración y llaves de seguridad.</p>
			</div>
		</div>
	{:else}
		<!-- Container filling the exact window size - Stable h-full -->
		<div class="relative z-10 w-full h-full flex flex-col bg-surface-1 overflow-hidden">
			<!-- Header -->
			<div class="flex-none px-6 py-4 border-b border-surface flex items-center justify-between">
				<div class="flex items-center gap-3">
					<div class="p-2 bg-primary/10 rounded-lg">
						<Shield class="w-6 h-6 text-primary" />
					</div>
					<div>
						<h2 class="text-lg font-bold text-primary">Configuración Inicial</h2>
						<p class="text-xs text-secondary">Configura la terminal y la seguridad del sistema</p>
					</div>
				</div>
			...
			<!-- Content Area (Stable) -->
			<div class="px-6 py-4 flex-1 flex flex-col justify-between overflow-hidden">
				{#if error}
					<div
				...
				<!-- Main Card -->
				<div class="bg-black/20 border border-white/5 rounded-xl p-4 flex flex-col gap-4">
					<!-- Inputs Grid (Side by Side) -->
					<div class="grid grid-cols-2 gap-4 flex-none">
						<div>
							<label for="terminalName" class={labelClass}>Nombre de la Terminal</label>
							<input
								id="terminalName"
								type="text"
						...
						<div>
							<label for="terminalLocation" class={labelClass}>Ubicación Física</label>
							<input
								id="terminalLocation"
								type="text"
					...
					<div class="flex flex-col min-h-0">
						...
						<div class="grid grid-cols-3 gap-2 mb-4 flex-none">
							...
							>
								<span class="font-bold text-[10px] uppercase tracking-wider">Nueva Key</span>
							</button>
							<button
								...
								'join'
									...
									: 'bg-surface-3 border-surface text-secondary hover:bg-surface-2 hover:text-white'}"
							>
								<span class="font-bold text-[10px] uppercase tracking-wider">Key Existente</span>
							</button>
							<button
								...
								'recovery'
									...
									: 'bg-surface-3 border-surface text-secondary hover:bg-surface-2 hover:text-white'}"
							>
								<span class="font-bold text-[10px] uppercase tracking-wider">Recuperar</span>
							</button>
						</div>
						...
						<div class="flex-none">
									...
									<div class="space-y-3 animate-in fade-in slide-in-from-bottom-2">
										...
										{:else}
											...
											>
												...
												<div>
													<p class="text-sm font-bold text-white">Llave Maestra Lista</p>
										...
										<div class="flex items-center justify-between gap-4 pt-1">
											<p class="text-[10px] text-secondary flex-1 italic leading-tight">
												Se recomienda generar fragmentos por si pierdes la llave.
											</p>
											<button
									...
									<div class="space-y-4 mb-4">
										<div>
											...
											<div class="flex gap-2">
												...
												<button
													type="button"
													onclick={() => (setupPassword = generateInstallPassword())}
							...
							{:else if securityMode === 'join'}
								<div class="space-y-4 animate-in fade-in slide-in-from-bottom-2">
									<div>
										<label for="import-pass" class={labelClass}>Contraseña del Archivo (.key)</label
										>
										<input
											id="import-pass"
											type="password"
							...
							{:else if securityMode === 'recovery'}
								<div class="space-y-4 animate-in fade-in slide-in-from-bottom-2 text-center">
									<p class="text-xs text-secondary px-6">
										Selecciona al menos 3 de los 5 fragmentos generados anteriormente para
										reconstruir la llave maestra.
									</p>
									<button
