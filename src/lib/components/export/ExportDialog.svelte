...
<script lang="ts">
	...
	import { X, FileText, Table2, FileSpreadsheet, Download, Settings, Columns } from 'lucide-svelte';
		...
		>
			...
			<div class="w-full max-w-sm flex flex-col">
				...
				<div class="flex-1 overflow-y-auto p-5 space-y-4">
					<!-- Formato -->
					<div>
						<span class="block text-xs font-medium text-[#8b949e] mb-2 uppercase tracking-wide">
							Formato
						</span>
					...
					{#if selectedFormat === 'pdf'}
						...
						>
							<div>
								<label for="orientation" class="block text-xs font-medium text-[#8b949e] mb-1.5">
									Orientación
								</label>
								<select
									id="orientation"
					...
					{#if selectedFormat === 'csv'}
						...
						>
							<div>
								<label for="delimiter" class="block text-xs font-medium text-[#8b949e] mb-1.5">
									Delimitador
								</label>
								<select
									id="delimiter"
							...
							<label class="flex items-center gap-2 cursor-pointer py-1.5">
								...
								/>
								<span class="text-sm text-[#e6edf3]">UTF-8 BOM (para Excel)</span>
							</label>
						</div>
					{/if}
				...
				>
					...
					{#if columnSelection.filter((c) => c.selected).length === 0}
						<div class="px-4 py-2.5 border-t border-[#30363d] bg-[#3b2f00]">
							<p class="text-xs text-[#d29922]">Selecciona al menos una columna</p>
						</div>
					{/if}
				</div>
			{/if}
