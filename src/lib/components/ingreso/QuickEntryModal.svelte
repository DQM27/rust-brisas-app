<script lang="ts">
	...
	async function handleSearch() {
		...
		try {
			...
			results = rawResults.filter((r) => allowedTypes.includes(r.tipo.toLowerCase()));
			highlightedIndex = 0;
		} catch (e) {
		...
		>
			...
			<div class="px-4 py-3 border-b border-white/5">
				...
				>
					...
					/>
					{#if loading}
						<div class="absolute right-3 animate-spin text-gray-500 text-xs">⏳</div>
					{/if}
				</div>
			</div>
                  ...
                  {i === highlightedIndex
							...
							>
								<div class="flex-shrink-0 text-gray-500">
									<User size={14} class={i === highlightedIndex ? 'text-white' : 'text-gray-500'} />
								</div>
								...
								<div class="flex-1 min-w-0">
									...
									<div class="flex items-center gap-3 text-[11px] opacity-70 mt-0.5">
										<span class="flex items-center gap-1 font-mono">
											<CreditCard size={10} />
											{person.cedula || 'N/A'}
										</span>
										<span class="truncate">| {person.empresaNombre || 'S/E'}</span>
									</div>
								</div>
			...
			<div class="px-4 py-2 bg-black/20 border-t border-white/5 flex justify-between items-center">
				...
				<div class="flex items-center gap-3">
					<span class="text-[9px] text-gray-600 flex items-center gap-1">
						<kbd class="px-1 py-0.5 bg-black/40 rounded border border-white/10 text-[8px]">ESC</kbd>
					</span>
					<span class="text-[9px] text-gray-600 flex items-center gap-1">
