<script lang="ts">
  ...
  // Suscribirse al store para actualizaciones en tiempo real
  ...
  async function handleConfirmSalida(event: CustomEvent) {
    ...
    const result = await ingresoService.registrarSalida({
      ...
      usuarioSalidaId: "usuario_actual_id", // TODO: Store auth
    });

    if (result.ok) {
      toast.success("Salida registrada");
      ingresoStore.remove(result.data.id); // Remover del store
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full p-4">
  ...
  <div
    class="lg:col-span-2 flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden"
  >
    <div
