<script lang="ts">
  ...
  // Helpers
  function getBloqueoTitle(bloqueo: MotivoBloqueo): string {
    switch (bloqueo.tipo) {
      case "lista_negra":
        return "⛔ Persona en Lista Negra";
      case "ingreso_activo":
        return "⚠️ Ya tiene un ingreso activo";
      case "gafetes_pendientes":
        return "🆔 Gafetes pendientes de devolución";
      default:
        return "❌ Ingreso No Autorizado";
    }
  }

  function getBloqueoDesc(bloqueo: MotivoBloqueo): string {
    switch (bloqueo.tipo) {
      case "lista_negra":
        return bloqueo.motivo;
      case "ingreso_activo":
        return "Debe registrar salida antes de volver a ingresar.";
      case "gafetes_pendientes":
        return `Tiene ${bloqueo.cantidad} gafete(s) sin devolver.`;
      default:
        return typeof bloqueo.tipo === "string"
          ? bloqueo.tipo
