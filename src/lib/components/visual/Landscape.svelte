<script lang="ts">
	...
	import { LANDSCAPE_PATHS, getLandscapeTheme, type LandscapeType } from './systems/landscapeData';
	...
	{#key type}
		<div class="absolute inset-0 w-full h-full" in:fade={{ duration: 1000 }}>
			...
			<svg
				...
				style="filter: brightness({adjustedBrightness}); color: {theme.colors[0]}"
			>
			...
			<svg
				...
				viewBox="0 0 1440 320"
				preserveAspectRatio="none"
				style="filter: brightness({adjustedBrightness}); color: {theme.colors[1]}"
			>
			...
			<svg
				...
				viewBox="0 0 1440 320"
				preserveAspectRatio="none"
				style="filter: brightness({adjustedBrightness}); color: {theme.colors[2]}"
			>
			...
			{#if showCityLights}
				...
				>
					...
					<defs>
						...
						>
							<rect x="2" y="2" width="4" height="4" fill="#fbbf24" fill-opacity="0.6" />
							<rect x="12" y="10" width="3" height="3" fill="#fbbf24" fill-opacity="0.4" />
						</pattern>
					</defs>
