...
import { TIME, STAR_CONFIG, seededRandom, randomRange } from '../constants';
	...
	for (let i = 0; i < STAR_CONFIG.COUNT; i++) {
		...
		stars.push({
			...
			twinkleSpeed: randomRange(
				STAR_CONFIG.TWINKLE_SPEED_RANGE[0],
				STAR_CONFIG.TWINKLE_SPEED_RANGE[1]
			)
		});
	}
...
export function updateStarSystem(
	...
	const shootingStars = state.shootingStars
		.map((star) =>
			updateShootingStar(star, render.deltaTime, canvas, render.starSettings?.shootingStarSpeed)
		)
