...
export function updateStarSystem(
  ...
  let shootingStars = state.shootingStars
    .map(star => updateShootingStar(star, render.deltaTime, canvas))
    .filter(star => star.active);
  
  // Maybe spawn new shooting star (only at night)
  if (visibility > 0.5 && Math.random() < 0.0003) { // ~1 every 30-60 seconds at 60fps
...
function createShootingStar(canvas: CanvasContext): ShootingStar {
  ...
  return {
    ...
    speed: STAR_CONFIG.SHOOTING_STAR_SPEED * (0.8 + Math.random() * 0.4),
...
function updateShootingStar(
  ...
  const speed = star.speed * (deltaTime / 16); // Normalize to 60fps
