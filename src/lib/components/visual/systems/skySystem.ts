...
export function renderSkySystem(
  state: SkyState, 
  render: RenderState, 
...
function calculateSkyPalette(hour: number): { palette: [string, string, string]; timeOfDay: TimeOfDay } {
  // Night (18:30 - 5:30)
  ...
  if (hour >= TIME.DUSK_START && hour < TIME.DUSK_END) {
    ...
    return { 
      palette: interpolateSkyPalette(SKY_PALETTES.evening, SKY_PALETTES.night, (progress - 0.5) * 2),
      timeOfDay: 'evening'
    };
  }
