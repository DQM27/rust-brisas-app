...
<script lang="ts">
	...
	const previewStart = $derived(`${prefix}${start.toString().padStart(padding, '0')}`);
	const previewEnd = $derived(`${prefix}${end.toString().padStart(padding, '0')}`);
	...
	>
		...
		>
			...
			<form onsubmit={handleSubmit} class="p-6 space-y-4">
				...
				>
					<p class="text-blue-700 dark:text-blue-400 font-medium mb-1">Resumen de generación:</p>
