...
<script lang="ts">
	...
	import { CreateUserSchema, type CreateUserForm } from '$lib/schemas/userSchema';
	...
	interface Props {
		...
		loading?: boolean;
		onSave: (data: CreateUserInput | UpdateUserInput) => Promise<boolean | void>;
		onClose: () => void;
		readonly?: boolean;
	...
	// El usuario está editando su propio perfil (usando prop explícita o comparación de IDs)
	const isSelf = $derived(isSelfEdit || (!!user && !!$currentUser && user.id === $currentUser.id));

	// Nombre completo para mostrar en el header
	const getFullName = (u: UserResponse | null | undefined) => {
		if (!u) return '';
		return [u.nombre, u.segundoNombre, u.apellido, u.segundoApellido].filter(Boolean).join(' ');
	};
	...
	const {
		...
		onUpdate: async ({ form: f }) => {
			...
			if (payloadData.fechaInicioLabores)
				payloadData.fechaInicioLabores = formatDateForBackend(payloadData.fechaInicioLabores);
			if (payloadData.fechaNacimiento)
				payloadData.fechaNacimiento = formatDateForBackend(payloadData.fechaNacimiento);
			if (payloadData.vencimientoPortacion)
				payloadData.vencimientoPortacion = formatDateForBackend(payloadData.vencimientoPortacion);
	...
	const currentRoleName = $derived(
		$form.roleId
			? (availableRoles.find((r) => r.id === $form.roleId)?.name ?? 'Cargando...')
			: 'Sin Rol'
	);
						...
						<div class="flex-1 p-6 space-y-4">
								...
								<div class="flex flex-col items-center justify-center mb-4">
									<div class="relative group">
										...
										>
											...
											{#if activeAvatar}
												<img src={activeAvatar} alt="Avatar" class="w-full h-full object-cover" />
											{:else}
												<div
													class="flex flex-col items-center justify-center text-gray-400 dark:text-gray-600"
												>
													<span class="text-3xl font-bold">
														{$form.nombre ? $form.nombre[0].toUpperCase() : '?'}{$form.apellido
							...
							>
								...
								<div class="space-y-3">
										...
										{#if !isSelf}
											<div class="relative">
												<label for="operacion" class={labelClass}
													>Operación / CDI <span class="text-red-500 ml-0.5">*</span></label
												>
												<button
													type="button"
													disabled={loading || readonly}
									...
									<div>
										<label for="email" class={labelClass}
											>Correo Electrónico <span class="text-red-500 ml-0.5">*</span></label
										>
										<input
											id="email"
											type="email"
								...
								<div class="space-y-3">
									<div class="grid grid-cols-2 gap-2">
										<div>
											<label for="vencimientoPortacion" class="{labelClass} whitespace-nowrap">
												Venc. Portación <span class="text-red-500 ml-0.5">*</span>
											</label>
											<input
												...
												onkeydown={(e) => handleDateTab(e, 'fechaInicioLabores')}
											/>
										...
										<div>
											<label for="fechaInicioLabores" class={labelClass}>Inicio Labores</label>
											<input
									...
									<div class="grid grid-cols-2 gap-2">
										...
										<div>
											<label for="fechaNacimiento" class={labelClass}>Fecha Nacimiento</label>
											<input
												id="fechaNacimiento"
												type="text"
										...
										<div>
											<label for="telefono" class={labelClass}>Tel. Personal</label>
											<input
												id="telefono"
												type="tel"
												value={$form.telefono}
												...
												disabled={loading || readonly}
												class="{inputClass} {getFieldStateClass('telefono', $form.telefono)}"
											/>
										</div>
									</div>
									...
									<div class="grid grid-cols-2 gap-2">
										...
										<div>
											...
											<input
												...
												oninput={(e) => handleGenericPhoneInput(e, 'contactoEmergenciaTelefono')}
												onkeydown={handlePhoneKeydown}
												placeholder="+506 8888-8888"
												disabled={loading || readonly}
												class="{inputClass} {getFieldStateClass(
								...
								{#if isEditMode && !isSelf && !readonly}
									<div class="col-span-1 md:col-span-2 flex items-center justify-between gap-4">
										...
										<div class="flex items-center gap-3">
											<label class="relative inline-flex items-center cursor-pointer">
												...
												/>
												<div
													class="w-9 h-5 bg-surface-3 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"
												></div>
											</label>
											<span class="text-xs font-medium text-gray-300">Forzar cambio de clave</span>
										</div>

										<!-- Button: Generar Temporal -->
										<button
											type="button"
											onclick={handleResetPasswordClick}
											disabled={loading}
											class="px-3 py-1.5 rounded bg-surface-2 border border-surface text-secondary hover:text-white hover:border-white/20 transition-all text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap"
										>
		...
		>
			<div class="text-center">
				...
				<p class="text-gray-500 dark:text-gray-400 text-sm mb-6">
					La contraseña ha sido generada exitosamente. Por favor compártela con el usuario.
				</p>
