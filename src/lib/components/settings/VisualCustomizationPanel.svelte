<script lang="ts">
	...
	export let mode: 'bokeh' | 'weather' | 'celestial' | 'stars' | 'clouds' | 'landscape' = 'bokeh'; // New prop
...
{#if !embedded}
	...
	>
		...
		>
			<!-- Header -->
			<div class="flex items-center justify-between p-4 border-b border-emphasis bg-surface-2">
				<div class="flex items-center gap-3">
					<div class="p-2 rounded-lg bg-pink-500/10 text-pink-500">
						<Sliders size={20} />
					</div>
					<div>
						<h2 class="text-lg font-bold text-primary">Personalización Avanzada</h2>
						<p class="text-xs text-secondary">Ajuste fino de efectos visuales</p>
					</div>
				</div>
				<button
			...
			<!-- Footer for Modal -->
			<div class="p-4 bg-surface-2 border-t border-emphasis flex justify-end gap-3">
				<button
...
{:else}
	<!-- Inline Version -->
	<div class="w-full bg-surface-2/50 rounded-xl border border-white/5 p-4 mt-2" transition:slide>
		{@render content()}
...
{#snippet content()}
		...
		<div class="space-y-4">
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateBokehCount($particleSettings.bokehCount)}
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateBokehOpacity($particleSettings.bokehMaxOpacity)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer"
				/>
			</div>
		...
		<div class="space-y-4">
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateWeatherDensity($particleSettings.weatherDensityMultiplier)}
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateWeatherSpeed($particleSettings.weatherSpeedMultiplier)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-500"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateWeatherSize($particleSettings.weatherSizeMultiplier)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-500"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateWeatherWind($particleSettings.weatherWindInfluence)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-500"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateWeatherTurbulence($particleSettings.weatherTurbulence)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-500"
				/>
			</div>
		</div>
		...
		<div class="space-y-4">
			...
			<div class="control-group">
				<div class="flex justify-between items-center mb-2">
					...
					<span
						...
						>{MOON_PHASES.find((p) => p.id === $particleSettings.moonPhase)?.name ??
							$particleSettings.moonPhase}</span
					>
				</div>
			...
			<div class="control-group mt-6">
                ...
                {$particleSettings.sunStyle === 'cloudy'
					...
					>
						<div class="relative">
							<Sun size={18} class="text-amber-400 absolute -top-1 -left-1 opacity-50" />
							<CloudRain size={18} />
						</div>
	...
	{:else if mode === 'stars'}
		...
		<div class="space-y-4">
			...
			<div class="control-group bg-yellow-400/10 border border-yellow-400/20 p-3 rounded-lg mb-4">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-2">
						<Zap size={16} class="text-yellow-400" />
						<span class="text-sm font-medium text-yellow-100">Lluvia de Meteoros</span>
					</div>
					<button
              ...
              {$particleSettings.meteorShowerEnabled ? 'bg-yellow-400' : 'bg-surface-3'}"
						onclick={particleSettings.toggleMeteorShower}
						aria-label="Toggle Meteor Shower"
					>
						<span
							class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform
              {$particleSettings.meteorShowerEnabled ? 'translate-x-6' : 'translate-x-1'}"
						></span>
					</button>
				</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateStarCount($particleSettings.starCountMultiplier)}
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateStarTwinkle($particleSettings.starTwinkleSpeed)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-amber-300"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateShootingStarFreq($particleSettings.shootingStarFrequency)}
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() =>
						particleSettings.updateShootingStarSpeed($particleSettings.shootingStarSpeed)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-yellow-200"
				/>
			</div>
		</div>
	{:else if mode === 'landscape'}
		<div class="space-y-4">
			...
			<p class="text-xs text-secondary/60 text-center mt-2 px-2">
				El paisaje cambia dinámicamente según la hora del día y la estación del año.
			</p>
		</div>
	{:else if mode === 'clouds'}
		...
		<div class="space-y-4">
			<div class="flex items-center gap-2 mb-2">
				...
				<h3 class="text-sm font-semibold text-primary uppercase tracking-wider">Nubes</h3>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateCloudOpacity($particleSettings.cloudOpacity)}
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateCloudCount($particleSettings.cloudCount)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-400"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateCloudWindSpeed($particleSettings.cloudWindSpeed)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-400"
				/>
			</div>
			...
			<div class="control-group">
				...
				<input
					...
					oninput={() => particleSettings.updateCloudTurbulence($particleSettings.cloudTurbulence)}
					class="w-full h-1.5 bg-surface-3 rounded-lg appearance-none cursor-pointer accent-blue-400"
				/>
			</div>
		</div>
