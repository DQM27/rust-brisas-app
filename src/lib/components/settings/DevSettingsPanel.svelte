<script lang="ts">
  ...
  import { modulesStore, type ModuleStatus } from "$lib/stores/modules";
  ...
  >
    {#each filteredModules as module (module.key)}
      ...
      >
        ...
        <div
          class="absolute left-0 top-0 bottom-0 w-1 {config.class.split(
            ' ',
          )[1] || 'bg-gray-500/20'}"
        ></div>
        ...
        <div class="pl-2 mt-4">
          ...
          <div class="relative">
            <select
              ...
              value={module.status}
              on:change={(e) =>
                handleStatusChange(module.key, e.currentTarget.value)}
            ...
            >
              ...
              <option
                ...
                class={!$currentUser?.isSuperuser
                  ? "opacity-30 text-gray-600"
                  : "text-yellow-400"}
              >
                🚧 En Construcción {!$currentUser?.isSuperuser
                  ? "(Solo GOD)"
                  : ""}
              </option>
              <option
                ...
                class={!$currentUser?.isSuperuser
                  ...
                  : "text-orange-400"}
              >
                🔧 En Mantenimiento {!$currentUser?.isSuperuser
                  ? "(Solo GOD)"
                  : ""}
              </option>
            </select>
