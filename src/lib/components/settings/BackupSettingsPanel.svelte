  ...
  <div class="max-w-2xl space-y-6">
    ...
    >
      ...
      <div class="p-4">
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-4">
          Exporta toda la base de datos a un archivo <code
            class="bg-gray-100 dark:bg-[#21262d] px-1.5 py-0.5 rounded text-xs"
            >.db</code
          > que contiene todos los registros actuales.
        </p>
        ...
        <button
          ...
          disabled={isBackingUp || !canUpdate}
        >
          {#if isBackingUp}
    ...
    >
      ...
      <div class="p-4">
        <!-- Warning -->
        <div
          class="flex gap-3 p-3 mb-4 rounded-md bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/30"
        >
          <AlertTriangle
            class="w-4 h-4 text-yellow-600 dark:text-yellow-500 flex-shrink-0 mt-0.5"
          />
          <div class="text-sm text-yellow-800 dark:text-yellow-200">
            <strong>Advertencia:</strong> Esta accion reemplazara todos los datos
            actuales. La aplicacion se reiniciara automaticamente.
          </div>
        </div>
        ...
        <button
          ...
          disabled={isRestoring || !canUpdate}
        >
          {#if isRestoring}
            <RefreshCw class="w-4 h-4 animate-spin" />
