...
<script lang="ts">
	...
	async function handleDelete(roleId: string) {
		if (!confirm('¿Estás seguro de que deseas eliminar este rol permanentemente?')) return;
	...
	function translateModule(module: string): string {
		return moduleTranslations[module.toLowerCase()] || module.replace(/_/g, ' ');
	}

	function toggleModulePermissions(perms: Permission[]) {
		const allSelected = perms.every((p) => formData.permissions.includes(p.id));
		if (allSelected) {
			formData.permissions = formData.permissions.filter((id) => !perms.find((p) => p.id === id));
		} else {
			const newIds = perms.map((p) => p.id).filter((id) => !formData.permissions.includes(id));
		...
		<div class="flex flex-col h-full" in:fade={{ duration: 200 }}>
			...
			<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					...
					<p class="text-gray-400 mt-1">Configura el acceso y control del sistema.</p>
				</div>
						...
						>
							...
							<div class="flex items-center gap-2 mb-6">
								...
								>
									...
									<span class="text-sm font-mono text-gray-300">{role.permissions.length}</span>
		...
		>
			...
			<div class="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
				<div>
					...
					<p class="text-gray-400 mt-1">Configura los detalles y permisos del rol.</p>
				</div>
			</div>
			...
			<div class="flex-1 overflow-y-auto custom-scrollbar pr-2">
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8 items-center">
					...
					<div class="space-y-4 max-w-md">
						...
						<div class="space-y-2">
							<label for="desc" class="block text-sm font-medium text-gray-300">Descripción</label>
							<textarea
								id="desc"
					...
					>
						...
						>
							<div class="flex items-center gap-2">
								...
								<h4 class="text-white font-medium text-sm">Resumen de Acceso</h4>
							</div>
							<span class="text-xs text-gray-500 font-mono">{formData.permissions.length} sel.</span
							>
						</div>
						...
						<div class="flex-1 overflow-y-auto custom-scrollbar p-4">
							{#if formData.permissions.length === 0}
								<div class="h-full flex items-center justify-center text-gray-500 text-xs italic">
									Selecciona permisos abajo para ver el resumen.
								</div>
							{:else}
								<div class="grid grid-cols-2 gap-x-6 gap-y-6">
									{#each [...getGroupedPermissions()] as [moduleName, perms]}
										{@const activePerms = perms.filter((p) => formData.permissions.includes(p.id))}
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-8">
						{#each [...getGroupedPermissions()] as [moduleName, perms]}
							...
							>
								...
								>
									<span class="font-medium text-gray-200 uppercase tracking-wide text-xs"
									...
									<button
										...
										class="text-xs px-2 py-1 rounded hover:bg-white/10 transition-colors {isModuleSelected(
											...
											: 'text-gray-500'}"
									>
										{isModuleSelected(perms) ? 'Desmarcar Todos' : 'Marcar Todos'}
									</button>
								</div>
								...
								<div class="p-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
									{#each perms as perm}
										...
										>
											<span class="relative z-10">{translateAction(perm.action)}</span>
										</button>
									{/each}
								</div>
							</div>
