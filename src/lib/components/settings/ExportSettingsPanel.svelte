							...
							<div class="flex items-start gap-3 flex-1">
								...
								<div class="flex-1">
									...
									<div class="mt-1 text-sm text-gray-500 dark:text-gray-400 space-y-1">
										<div>
											Formato: <span class="font-medium">{profile.format.toUpperCase()}</span>
										</div>
										{#if profile.format === 'pdf' && profile.pdf_design}
											...
											<div class="flex items-center gap-2">
												<div
													...
													style="background: {profile.pdf_design.colors.header_fill};"
												></div>
												<div
													class="w-4 h-4 rounded border"
...
{#if showProfileEditor && editingProfile}
	<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" transition:fade>
		...
		>
			...
			<div class="flex-1 overflow-y-auto p-6 space-y-5">
				...
				{#if editingProfile.format === 'pdf' && editingProfile.pdf_design}
					<div class="space-y-4 p-4 bg-gray-50 dark:bg-[#161b22] rounded-lg">
						...
						<div class="grid grid-cols-2 gap-4">
							...
							<div>
								<label for="pdfOrientation" class="block text-sm mb-2">Orientación</label>
								<select
									id="pdfOrientation"
						...
						<div class="grid grid-cols-2 gap-4">
							<div>
								<label for="pdfMarginX" class="block text-sm mb-2">Margen X</label>
							...
							<div>
								<label for="pdfMarginY" class="block text-sm mb-2">Margen Y</label>
								<div class="flex gap-2">
									<input
										id="pdfMarginY"
										type="number"
				...
				{:else if editingProfile.format === 'csv' && editingProfile.csv_options}
					<div class="space-y-4 p-4 bg-gray-50 dark:bg-[#161b22] rounded-lg">
						...
						<div>
							<label for="csvDelimiter" class="block text-sm mb-2">Delimitador</label>
							<select
								id="csvDelimiter"
			...
			<!-- Footer -->
			<div class="bg-gray-50 dark:bg-[#161b22] px-6 py-4 border-t flex gap-3 justify-end">
				<button
					onclick={() => {
						showProfileEditor = false;
						editingProfile = null;
