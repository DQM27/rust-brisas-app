...
<script lang="ts">
	...
	import { vehiculoSchema, type VehiculoFormData } from '$lib/schemas/vehiculoSchema';
	...
	const { form, errors, constraints, enhance, reset } = superForm<VehiculoFormData>(defaultValues, {
		SPA: true,
	...
	async function saveData(data: VehiculoFormData) {
		...
		try {
			...
			globalError = 'Error al guardar el vehículo. Verifique si la placa ya existe.';
		} finally {
			submitting = false;
		}
	}
...
{#if show}
	...
	>
		...
		<div class="absolute inset-0 bg-black/60" role="button" tabindex="0" onclick={onClose}></div>
			...
			<div class="flex-1 overflow-y-auto p-6 space-y-6">
				...
				{#if !showForm}
					<div class="space-y-3">
						...
						{#if loading}
							<div class="text-center py-8 text-secondary text-sm">Cargando vehículos...</div>
				...
				{:else}
					...
					>
						<h3 class="text-sm font-semibold text-primary border-b border-surface pb-2">
							...
							<div class="space-y-1 relative">
								...
								<div class="relative">
									<button
										...
										class="{selectClass} {getFieldStateClass('tipoVehiculo', $form.tipoVehiculo)}"
									>
										<span class={$form.tipoVehiculo ? 'text-white' : 'text-gray-500'}>
									...
									{#if showTipoDropdown}
										...
										<div
											...
											role="button"
											tabindex="-1"
											onkeydown={(e) => e.key === 'Escape' && (showTipoDropdown = false)}
										></div>
									{/if}
								</div>
								<!-- Hidden input for binding/validation compatibility if needed, though bind:value updates form -->
								<input type="hidden" name="tipoVehiculo" bind:value={$form.tipoVehiculo} />
							...
							<div class="grid grid-cols-2 gap-4">
								<div class="space-y-1">
									...
									<input
										...
										placeholder="ABC-123"
										class="{inputClass} uppercase {getFieldStateClass('placa', $form.placa)}"
										disabled={submitting}
										{...$constraints.placa}
									/>
									{#if $errors.placa}
								...
								<div class="space-y-1">
									...
									<input
										...
										placeholder="Toyota"
										class="{inputClass} {getFieldStateClass('marca', $form.marca)}"
										disabled={submitting}
										{...$constraints.marca}
									/>
									{#if $errors.marca}
							...
							<div class="grid grid-cols-2 gap-4">
								<div class="space-y-1">
									...
									<input
										...
										placeholder="Corolla"
										class="{inputClass} {getFieldStateClass('modelo', $form.modelo)}"
										disabled={submitting}
										{...$constraints.modelo}
									/>
									{#if $errors.modelo}
								...
								<div class="space-y-1">
									...
									<input
										...
										placeholder="Blanco"
										class="{inputClass} {getFieldStateClass('color', $form.color)}"
										disabled={submitting}
										{...$constraints.color}
									/>
									{#if $errors.color}
