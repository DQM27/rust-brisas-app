<script lang="ts">
	...
	interface Props {
		...
		loading?: boolean;
		onSave: (data: CreateProveedorInput | UpdateProveedorInput) => Promise<boolean | void>;
		onClose: () => void;
		readonly?: boolean;
	}
	...
	const { form, errors, constraints, enhance, reset, validate } = superForm<CombinedForm>(
		emptyFormData,
		{
			SPA: true,
			validators: zod4(proveedor ? UpdateProveedorSchema : CreateProveedorSchema),
			dataType: 'json',
