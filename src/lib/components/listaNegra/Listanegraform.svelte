  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    switch (event.key) {
      ...
      case "Tab":
        ...
        if (
          highlightedIndex >= 0 &&
          highlightedIndex < filteredContratistas.length
        ) {
...
<div class="flex min-h-full items-center justify-center p-6">
  <div
    class="relative z-10 w-full max-w-2xl rounded-xl bg-[#1e1e1e] shadow-2xl ring-1 ring-white/10"
  >
    <div class="border-b border-white/10 px-8 py-5">
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-lg bg-red-500/10"
        >
          <svg
      ...
      <div class="mb-6 space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <button
            ...
            disabled={loading}
            class="flex items-center justify-center gap-2 rounded-lg border px-4 py-3 text-sm font-medium transition-all {modoRegistro ===
            'existente'
...
{#if showConfirmModal}
  <div
    class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 transition-opacity"
    transition:fade
  >
    <div
      class="bg-[#1e1e1e] rounded-xl shadow-2xl max-w-sm w-full p-6"
    ...
    >
      ...
      <p class="text-sm text-gray-300 mb-6">
        ...
        El motivo y las observaciones para esta acción serán registradas en el
        historial de la persona.
      </p>

      <div class="space-y-4">
        <div class="space-y-1.5">
          <label for="unblockMotivo" class="text-sm font-medium text-gray-300"
            >Motivo del Desbloqueo <span class="text-red-500">*</span></label
          >
          <textarea
            id="unblockMotivo"
            bind:value={motivoBloqueo}
