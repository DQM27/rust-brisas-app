...
<script lang="ts">
  ...
  function handleClick(): void {
    ...
    console.log(
      `Toggle panel de inspección: ${newVisible ? "visible" : "oculto"}`,
    );
  ...
  const responsiveClasses =
    variant === "default" ? "md:gap-1.5 md:px-2 md:text-xs" : "";
</script>

<button
  class="{baseClasses} {variantClasses[
    variant
  ]} {stateClasses} {activeClasses} {responsiveClasses} rounded-sm"
  class:disabled
...
>
  ...
  >
    {#if visible}
      <EyeOff
        {size}
        class="transition-transform duration-150 {!disabled
          ? 'hover:scale-110'
          : ''}"
      />
    {:else}
  ...
  {#if showLabel && variant !== "icon-only"}
    <span
      class="font-medium leading-none {variant === 'default'
        ? 'max-md:hidden'
        : ''}"
    >
      {#if variant === "compact"}
