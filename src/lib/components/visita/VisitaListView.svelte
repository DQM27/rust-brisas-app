<script lang="ts">
	...
	import type { CitaPopulated, CreateCitaInput, CreateVisitanteInput } from '$lib/types/cita';
	...
	// Guardar (Crear)
	async function handleSave(data: { cita: CreateCitaInput; visitante: CreateVisitanteInput }) {
		modalLoading = true;
		try {
	...
	async function handleRegistrarIngreso(visita: CitaPopulated) {
		const gafete = prompt(`Ingrese número de gafete para ${visita.visitante_nombre_completo}:`);
		if (!gafete) return;
		...
		return {
			...
			singleSelect: [
				...
				},
				{
					...
					onClick: async () => {
						if (!selected) return;
						if (!confirm(`¿Cancelar visita de ${selected.visitante_nombre_completo}?`)) return;
