<script lang="ts">
	import { X, Save, Calendar, User, Clock, Building, MapPin, Car } from 'lucide-svelte';
				...
				<section class="space-y-4">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
						<div class="form-group">
							...
							/>
							{#if errors.fecha}<span class={errorClass}>{errors.fecha}</span>{/if}
						</div>
						<div class="form-group">
							...
							/>
							{#if errors.hora}<span class={errorClass}>{errors.hora}</span>{/if}
						</div>
						<div class="form-group md:col-span-2 lg:col-span-2">
							<label for="anfitrion" class={labelClass}>Anfitrión (Empleado) *</label>
							<div class="relative">
								...
								<input
									...
									class="{inputClass} pl-9 {errors.anfitrion ? inputErrorClass : ''}"
									placeholder="A quien visita"
								/>
							</div>
							{#if errors.anfitrion}<span class={errorClass}>{errors.anfitrion}</span>{/if}
						</div>

						<div class="form-group md:col-span-2">
							<label for="areaVisitada" class={labelClass}>Área / Departamento *</label>
							<div class="relative">
								...
								<input
									...
									class="{inputClass} pl-9 {errors.areaVisitada ? inputErrorClass : ''}"
									placeholder="Ej. Ventas"
								/>
							</div>
							{#if errors.areaVisitada}<span class={errorClass}>{errors.areaVisitada}</span>{/if}
						</div>
						<div class="form-group md:col-span-2">
				...
				<section class="space-y-4">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						<div class="form-group">
							...
							/>
							{#if errors.cedula}<span class={errorClass}>{errors.cedula}</span>{/if}
						</div>
						<div class="form-group">
							...
							/>
							{#if errors.nombre}<span class={errorClass}>{errors.nombre}</span>{/if}
						</div>
						<div class="form-group">
							...
							/>
							{#if errors.apellido}<span class={errorClass}>{errors.apellido}</span>{/if}
						</div>

						<!-- Opcionales -->
						<div class="form-group">
							<label for="segundoNombre" class="{labelClass} text-gray-500">Segundo Nombre</label>
							<input
								id="segundoNombre"
								type="text"
							...
							/>
							{#if errors.segundoNombre}<span class={errorClass}>{errors.segundoNombre}</span>{/if}
						</div>
						<div class="form-group">
							...
							<input
								...
								class="{inputClass} bg-gray-50 dark:bg-white/5"
							/>
							{#if errors.segundoApellido}<span class={errorClass}>{errors.segundoApellido}</span
								>{/if}
						</div>
						...
						<div class="form-group">
							...
							<div class="relative">
								<Building class="absolute left-3 top-2.5 text-gray-400" size={16} />
								<input
									id="empresa"
									type="text"
							...
							>
								<div class="form-group">
									...
									<input
										...
										class="{inputClass} uppercase font-mono {errors.placa ? inputErrorClass : ''}"
										placeholder="ABC-123"
									/>
									{#if errors.placa}<span class={errorClass}>{errors.placa}</span>{/if}
