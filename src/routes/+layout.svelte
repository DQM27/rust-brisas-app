			...
			if (screensaverActive) {
				...
				try {
					...
					if (isCurrentlyFullscreen) {
						...
						console.log('[Layout] Screensaver deactivated - exiting fullscreen');
					}
				} catch (e) {
	...
	onMount(() => {
		...
		const handleInteraction = () => {
			...
			if (ignoreInteractions) {
				console.log('[Layout] Ignoring interaction during screensaver activation cooldown');
				return;
			}

			if ($isScreensaverActive && !$awaitingScreensaverPassword) {
				console.log('[Layout] User interaction detected - attempting to exit screensaver');
				attemptExitScreensaver();
			}
		};
		...
		const unsubscribe = isScreensaverActive.subscribe((active) => {
			if (active) {
				console.log('[Layout] Screensaver activated - ignoring interactions for 500ms');
